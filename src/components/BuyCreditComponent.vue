...
const isPopular = (index: number) => productStore.products.length > 0 && index === Math.floor(productStore.products.length / 2)
...
<template>
  <Dialog v-model:open="useUserStore().buyCredits">
    <DialogTrigger as-child>
      <button
        class="text-primary rounded-full hover:bg-primary/10 transition-colors p-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1"
        :aria-label="$t('buyCredits.triggerLabel')">
        <Plus class="size-4" aria-hidden="true" />
      </button>
    </DialogTrigger>
    <DialogContent class="sm:max-w-[90dvw] max-w-dvw max-h-dvh p-6 overflow-auto" role="dialog" aria-modal="true"
      ...
      <DialogHeader class="text-center space-y-2 pb-2">
        <div class="flex items-center justify-center gap-2">
          ...
          <DialogTitle id="credits-dialog-title" class="text-2xl font-bold tracking-tight">{{
            $t('buyCredits.dialogTitle') }}</DialogTitle>
          <Sparkles class="size-5 text-primary/80" aria-hidden="true" />
        </div>
      ...
      <ul class="grid grid-cols-1 sm:grid-cols-3 gap-7" :aria-label="$t('buyCredits.plansLabel')" role="list">
        <li v-for="(prod, index) in productStore.products" :key="prod.id" role="listitem">
          <Card
            :class="['relative flex flex-col h-full transition-all duration-200 hover:shadow-md hover:-translate-y-1', isPopular(index) ? 'border-primary shadow-primary/50 scale-[1.03] z-10' : 'border-border']">
            <div v-if="isPopular(index)" class="absolute -top-3 left-1/2 -translate-x-1/2"
              :aria-label="$t('buyCredits.mostPopular')">
            ...
            <CardHeader class="pb-2 pt-6 text-center">
              ...
              <p class="text-sm text-muted-foreground font-medium">{{ $t('buyCredits.credits', { count: prod.credits })
              }}</p>
            </CardHeader>
            <CardContent class="flex-1 flex flex-col items-center justify-center gap-1 py-4">
              <p class="text-4xl font-extrabold tracking-tight" :class="isPopular(index) ? 'text-primary' : ''">{{
                formatPrice(prod) }}</p>
              <p class="text-xs text-muted-foreground">{{ pricePerCredit(prod) }}</p>
            </CardContent>
            <CardFooter class="pt-0 pb-4">
              <Button class="w-full gap-2 transition-all duration-200"
                :class="isPopular(index) ? 'bg-primary hover:bg-primary shadow-sm shadow-primary/30' : 'variant-outline'"
                :variant="isPopular(index) ? 'default' : 'outline'" :disabled="loading === prod.id"
                :aria-busy="loading === prod.id"
                :aria-label="`${$t('buyCredits.buyNow')} ${prod.name} — ${formatPrice(prod)}`"
                @click="handlePurchase(prod)">
