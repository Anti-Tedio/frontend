...
const openCreditsExhausted = ref(false)
...
<template>
  ...
  <nav class="flex items-center" :aria-label="$t('nav.home')">
    <div class="hidden md:flex items-center gap-6" role="menubar">
      <template v-if="token">
        <div class="flex items-center bg-primary/10 rounded-2xl py-1 px-2 gap-1.5 text-sm text-primary"
          :aria-label="$t('nav.credits', { count: userStore.credits ?? 0 })">
          <Zap class="fill-primary text-primary w-4" aria-hidden="true" />
          <span class="font-medium">{{ userStore.credits ?? 0 }}</span>
          <BuyCreditComponent />
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" class="w-40">
            <DropdownMenuItem v-for="lang in languages" :key="lang.code" class="flex items-center gap-2 cursor-pointer"
              @click="setLocale(lang.code)">
              <span>{{ lang.flag }}</span>
              <span class="flex-1 text-sm">{{ lang.label }}</span>
              <Check v-if="locale && locale === lang.code" class="w-3.5 h-3.5 text-primary" aria-hidden="true" />
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <template v-else>
        <a href="/" role="menuitem" class="text-sm text-zinc-600 hover:text-primary transition-colors no-underline"
          @click.prevent="navigate('home')">
          {{ $t('nav.home') }}
        </a>
        <a href="/about" role="menuitem"
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" class="w-40">
            <DropdownMenuItem v-for="lang in languages" :key="lang.code" class="flex items-center gap-2 cursor-pointer"
              ...
              <span>{{ lang.flag }}</span>
              <span class="flex-1 text-sm">{{ lang.label }}</span>
              <Check v-if="locale === lang.code" class="w-3.5 h-3.5 text-primary" aria-hidden="true" />
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <button
          ...
          @click="useUserStore().popUpLogin = true" :aria-label="$t('auth.signInButton')">
          {{ $t('nav.signIn') }}
        </button>
      </template>
    </div>
    ...
    <div class="flex md:hidden items-center gap-3">
      <Sheet v-model:open="isSheetOpen">
        <SheetTrigger class="text-primary bg-primary/15 rounded-full flex p-2 hover:bg-primary/25 transition-colors"
          :aria-label="$t('nav.openMenu')" :aria-expanded="isSheetOpen">
          <Menu class="w-5 h-5" aria-hidden="true" />
        </SheetTrigger>

        <SheetContent side="right" class="w-72 flex flex-col pt-10 px-0" role="dialog" :aria-label="$t('nav.openMenu')">
          <div class="px-6 pb-4">
            <template v-if="token">
          ...
          <nav class="flex flex-col flex-1 px-2 py-3 gap-1">
            ...
            <button v-for="lang in languages" :key="lang.code"
              class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors w-full text-left" :class="locale === lang.code
                ? 'bg-primary/10 text-primary font-medium'
                : 'text-zinc-700 hover:bg-primary/30 hover:text-primary'
                " @click="setLocale(lang.code)" :aria-pressed="locale === lang.code">
              <Globe class="w-4 h-4" aria-hidden="true" />
            ...
            <template v-if="token">
              <SheetClose as-child>
                <button
                  ...
                  @click="handleLogout" :aria-label="$t('nav.signOut')">
