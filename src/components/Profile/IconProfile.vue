...
<template>
  <DropdownMenu>
    <DropdownMenuTrigger class="pointer-events-none sm:pointer-events-auto focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1 rounded-full" :aria-label="$t('nav.accountMenu', { name: userName })" aria-haspopup="menu">
      <Avatar v-if="avatarUrl" class="w-13 h-13 md:w-10 md:h-10">
    ...
    <DropdownMenuContent align="end" class="w-44" role="menu">
      ...
      <DropdownMenuItem class="flex items-center gap-2 cursor-pointer text-red-500 focus:text-red-500 focus:bg-red-50" role="menuitem" @click="$emit('logout')">
