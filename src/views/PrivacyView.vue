...
<template>
  <div class="max-w-3xl mx-auto py-12 px-4 animate-in fade-in duration-500">
    ...
    <header class="text-center space-y-4 mb-12">
      <div class="inline-flex p-4 bg-primary/10 rounded-full text-primary mx-auto" aria-hidden="true">
        <ShieldCheckIcon :size="40" />
      </div>
      <h1 class="text-4xl font-bold tracking-tight text-slate-900">
        {{ $t('privacy.pageTitle') }}
      </h1>
      <p class="text-slate-400 text-sm">{{ $t('privacy.lastUpdated') }}</p>
    ...
    <footer class="mt-12 p-6 rounded-2xl bg-primary/5 border border-primary/10 text-center space-y-2">
      <p class="font-bold text-slate-700">{{ $t('privacy.contactQuestion') }}</p>
