...
function disabledBtnNext() {
  if (userStore.credits < 1) return true
  if (!categoriestore.selected) return true
  return false
}
...
<template>
  <div class="animate-in fade-in duration-500">
    <button
      ...
      @click="router.go(-1)" :aria-label="$t('selectCategory.back')">
    ...
    <section class="flex flex-col items-center gap-15 mt-xs w-full">
      <ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-4xl mx-auto px-4" role="radiogroup"
        :aria-label="$t('selectCategory.categoriesLabel')">
        <li v-for="cat in categoriestore.categories" :key="cat.id" v-show="cat.isActive"
          @click="categoriestore.selectCategory(cat)" @keydown.enter="categoriestore.selectCategory(cat)"
          @keydown.space.prevent="categoriestore.selectCategory(cat)" role="radio" :aria-checked="cat.select"
          :aria-label="`${cat.title}${cat.select ? ' — ' + $t('selectCategory.selected') : ''}`" tabindex="0"
          ...
          <div class="flex flex-col flex-grow">
            <span class="font-bold text-lg leading-tight transition-colors uppercase"
              :class="cat.select ? 'text-primary' : 'text-slate-700'">{{ cat.title[locale || 'en'] }}</span>
            <span v-if="cat.select"
              class="text-xs font-semibold text-primary uppercase tracking-wider animate-in fade-in slide-in-from-left-2"
              aria-hidden="true">{{ $t('selectCategory.selected') }}</span>
          </div>
          <div v-if="cat.select"
      ...
      <button
        ...
        :class="categoriestore.selected && userStore.credits > 0
          ? 'bg-primary hover:bg-primary/90 hover:scale-[1.02] shadow-md shadow-primary/25'
          : 'bg-primary/40 cursor-not-allowed'
          " :disabled="disabledBtnNext()" :aria-label="categoriestore.selected?.title['en'] === 'video game'
